...
We use the C4 Model to maintain a consistent language across all diagrams:
* **L1: System Context:** How the system interacts with users and external dependencies.
* **L2: Containers:** High-level breakdown into Web Apps, APIs, Databases, and Messaging Buses.
* **L3: Components:** (Required when a container exceeds 5+ internal modules or when onboarding feedback indicates the L2 view is insufficient.) Internal structural breakdown for highly complex containers.
...
* **Threat Modeling:** Required when introducing a new external-facing surface, a new authentication/authorization flow, or a new third-party integration. Use STRIDE or a lightweight equivalent.
* **Dependency Scanning:** Automated SAST and SCA (Software Composition Analysis) scans run on every Pull Request. High-severity findings block merge.
* **Secret Management:** All secrets, keys, and connection strings are stored in an approved vault (e.g., Azure Key Vault). Secrets **must never** be committed to source control — even temporarily.
* **Least Privilege:** Service accounts and API keys are scoped to the minimum permissions required.
* **Why?** A security incident costs 100x more than a security review. We shift security left because fixing vulnerabilities in production is fixing them too late.
